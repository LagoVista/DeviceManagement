...
namespace LagoVista.IoT.DeviceManagement.Rpc.Tests.Support
{
    ...
    public class Proxy : DispatchProxy
    {
        ...
        private readonly static MethodInfo _returnTypedValueInfo = typeof(IResponse).GetMethod(nameof(IResponse.GetTypedReturnValue), BindingFlags.Instance | BindingFlags.Public);
        ...
        protected override object Invoke(MethodInfo targetMethod, object[] args)
        {
            ...
            var typedReturnValueMethod = GetTypedReturnValueMethod(targetMethod);
            //var returnValue = typedReturnValueMethod.Invoke(response, null);
        ...
        private MethodInfo GetTypedReturnValueMethod(MethodInfo targetMethod)
        {
            var typeArgs = targetMethod.ReturnType.IsGenericType 
                ? targetMethod.ReturnType.GetGenericArguments() 
                : new Type[] { targetMethod.ReturnType };
            var method = _returnTypedValueInfo.MakeGenericMethod(typeArgs);
            return method;
        }
        ...
        private MethodInfo GetTaskFromResultMethod(MethodInfo targetMethod)
        {
            ...
            {
                var typeArgs = targetMethod.ReturnType.GetGenericArguments();
                method = typeArgs.Length > 0
                    ? _fromResultMethodInfo.MakeGenericMethod(typeArgs)
                    : _fromResultMethodInfo.MakeGenericMethod();
            }
            else if (targetMethod.ReturnType == typeof(Task))
            {
                method = _fromResultMethodInfo.MakeGenericMethod(new Type[] { typeof(object) });
            }
            return method;
        }
    }
